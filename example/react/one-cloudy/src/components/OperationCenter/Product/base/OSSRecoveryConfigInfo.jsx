...
class OSSRecoveryConfigInfo extends React.PureComponent {
  constructor (props) {
    super(props)
    this.state = {}
  }
  ...
  pasteClick = (id) => {
    ...
    form.setFieldsValue({
      [`${listName}.${id}.${PF.BACKUP_TIME_INTERVAL}`]: operateData[
        PF.BACKUP_TIME_INTERVAL
      ],
  ...
  copyClick = (id) => {
    ...
    const operateData = {}
    const listName = record.disabled ? 'backupOpenList' : PF.RESOURCE_DETAIL_LIST
    operateData[PF.BACKUP_TIME_INTERVAL] = form.getFieldValue(
      `${listName}.${id}.${PF.BACKUP_TIME_INTERVAL}`,
    )
    ...
    return (
      <div style={{ marginLeft: -80, marginTop: 50 }}>
        ...
        <Number
          id={
            record.disabled
              ? `backupOpenList.${id}.${PF.BACKUP_TIME_INTERVAL}`
              : `${PF.RESOURCE_DETAIL_LIST}.${id}.${PF.BACKUP_TIME_INTERVAL}`
          }
          label="时间间隔"
          max={1825}
          form={form}
          ...
          unit="天备份一次"
          disabled={!!record.disabled}
          initialValue={record.backupTimeInterval}
        />
...
export default OSSRecoveryConfigInfo
